...
def _load_sites_from_yaml(config_path: Path) -> list[LunarSite]:
...
def parse_roi(roi_raw: dict, *, default_use_full: bool = True) -> ROI:
    ...
    return ROI(use_full=use_full, bounding_box=bounding_box)
...
def main(argv: list[str] | None = None) -> None:
    ...
    if args.command == "site":
        ...
        sites = _load_sites_from_yaml(Path(args.config))
